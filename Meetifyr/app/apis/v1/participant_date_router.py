...
from app.services.participant_date_mysql import (
...
edgedb_router = APIRouter(
    prefix="/v1/edgedb/participant_dates",
    tags=["ParticipantDate"],
)
mysql_router = APIRouter(
    prefix="/v1/mysql/participant_dates",
    tags=["ParticipantDate"],
)
...
async def api_turn_on_date_mysql(
    ...
    await service_turn_on_participant_date_mysql(
        turn_on_participant_day_request.participant_date_id
    )
    meeting = await service_get_meeting_mysql(
        turn_on_participant_day_request.meeting_url_code
    )
    if meeting is None:
        raise HTTPException(
...
async def api_turn_off_date_mysql(
    ...
    await service_turn_off_participant_date_mysql(
        turn_off_participant_day_request.participant_date_id
    )

    meeting = await service_get_meeting_mysql(
        turn_off_participant_day_request.meeting_url_code
    )
    if meeting is None:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND,
...
async def api_star_date_mysql(
    ...
    await service_star_participant_date_mysql(
        star_participant_day_request.participant_date_id
    )
    meeting = await service_get_meeting_mysql(
        star_participant_day_request.meeting_url_code
    )
    if meeting is None:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND,
...
async def api_unstar_date_mysql(
    ...
    await service_unstar_participant_date_mysql(
        unstar_participant_day_request.participant_date_id
    )
    meeting = await service_get_meeting_mysql(
        unstar_participant_day_request.meeting_url_code
    )
    if meeting is None:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND,
