...
class ParticipantDateModel(BaseModel, Model):
    participant: fields.ForeignKeyRelation[
        ParticipantModel
    ...
    @classmethod
    async def on(
        cls, participant_date_id: int
    ) -> None:
        await cls.filter(
            id=participant_date_id
        ).update(enabled=True)
    ...
    async def off(
        ...
        await cls.filter(
            id=participant_date_id
        ).update(enabled=False, starred=False)
    ...
    async def star(
        ...
        await cls.filter(
            id=participant_date_id
        ).update(enabled=True, starred=True)
