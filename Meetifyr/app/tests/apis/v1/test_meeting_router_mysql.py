...
class TestMeetingRouter(TestCase):
    async def test_api_create_meeting_mysql(
        self,
    ) -> None:
        # When
        ...
        self.assertTrue(
            await MeetingModel.filter(
                url_code=url_code
            ).exists()
        )

    async def test_api_get_meeting_mysql(
        self,
    ) -> None:
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(
            ...
            await client.patch(
                f"/v1/mysql/meetings/{url_code}/title",
                json={"title": (title := "abc")},
            )

            # When
            response = await client.get(
                f"v1/mysql/meetings/{url_code}"
            )
        ...
        self.assertEqual(
            response_body["url_code"], url_code
        )
        self.assertEqual(
            response_body["start_date"],
            start_date,
        )
        self.assertEqual(
            response_body["end_date"], end_date
        )
        self.assertEqual(
            response_body["location"], "test"
        )
        self.assertEqual(
            response_body["title"], title
        )
        self.assertEqual(
            len(response_body["participants"]), 1
        )
        participant = response_body[
            "participants"
        ][0]
        self.assertEqual(
    ...
    async def test_api_get_meeting_mysql_404(
        self,
    ) -> None:
        ...
        async with httpx.AsyncClient(
            ...
            # When
            response = await client.get(
                f"v1/mysql/meetings/{invalid_url_code}"
            )
        ...
        self.assertEqual(
            ...
            "meeting with url_code: invalid_url_code not found",
        )

    async def test_api_update_meeting_date_range_mysql(
        self,
    ) -> None:
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(
        ...
        self.assertEqual(
            response_body["end_date"],
            "2025-10-22",
        )
        meeting = await MeetingModel.filter(
            url_code=url_code
        ).get()
        self.assertEqual(
            meeting.start_date, date(2025, 10, 12)
        )
        self.assertEqual(
            meeting.end_date, date(2025, 10, 22)
        )
    ...
    async def test_can_not_update_meeting_does_not_exists(
        self,
    ) -> None:
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(
        ...
        assert (
            response_body["detail"]
            == "meeting with url_code: invalid_url not found"
        )

    async def test_api_update_meeting_title(
        self,
    ) -> None:
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(
        ...
        meeting = await MeetingModel.get(
            url_code=url_code
        )
    ...
    async def test_api_update_meeting_location(
        self,
    ) -> None:
        # given
        async with httpx.AsyncClient(
